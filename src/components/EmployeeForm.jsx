...
const EmployeeForm = ({ fetchEmployees, selectedEmployee, clearSelection }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (selectedEmployee) {
        await axios.put(`https://employee-server-kgkg.onrender.com/allEmployees/${employee.id}`, employee);
      } else {
        await axios.post("https://employee-server-kgkg.onrender.com/allEmployees", employee);
      }
      fetchEmployees();
      clearSelection();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="text-center mb-4 w-50">
      ...
      <button type="submit" className="btn mt-3 btn-success">{selectedEmployee ? 'Update' : 'Add'} Employee</button>
    </form>
  );
};
